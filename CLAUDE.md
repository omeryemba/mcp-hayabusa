...
- **`knowledge.py`** — the detection knowledge base backing the `hayabusa://` resources. Like `get_hayabusa_rules`, it never shells out: `load_rule_catalog()` parses every Sigma rule YAML file under a rules directory, defaulting to `<hayabusa binary's directory>/rules` and validated through the same `_require_existing_path` from `hayabusa.py`, skipping and counting malformed files in `parse_errors` rather than raising. Every public function here takes an optional `rules_dir` override, same as `get_hayabusa_rules`, but the `server.py` resource registrations don't expose it as a URI parameter (a filesystem path doesn't fit naturally into a `hayabusa://` URI segment) — resources always read the default rules dir; a custom `rules_dir` still works by calling `knowledge.py` functions directly or by using the `get_hayabusa_rules` tool. Everything else here is aggregation over that parsed catalog, done in Python, not by hayabusa:
  - `rules_index()` groups rules by their top two path segments (e.g. `sigma/builtin`) into a browsable, bounded-per-category catalog.
  - `get_rule(rule_id)` looks up one rule's full detail by its Sigma `id` field.
  - `list_attack_techniques()` / `get_attack_technique(technique_id)` / `list_attack_tactics()` derive ATT&CK coverage entirely from each rule's own `tags:` field (e.g. `attack.t1059.001` for a technique, `attack.execution` for a tactic — see the regexes in `knowledge.py` for exactly what is/isn't matched, e.g. group/software tags like `attack.g0007` are excluded from both). There is no bundled MITRE ATT&CK reference dataset; technique/tactic names are whatever hayabusa's rule authors wrote as tags, so this only ever reflects what's actually installed, never goes stale against upstream ATT&CK content, but also can't show a technique that has zero detecting rules.
  - Enrichment is deliberately narrow, for the same no-bundled-dataset reason: `_mitre_url(technique_id)` is *computed* from the ID (`T1059.001` → `https://attack.mitre.org/techniques/T1059/001/`), not looked up, so it needs no data file and can't go stale. `_TACTIC_DISPLAY_NAMES` is a small hardcoded dict of the 14 Enterprise ATT&CK tactic names (`credential-access` → "Credential Access") — safe to hardcode because that list has been stable across ATT&CK versions, unlike the much larger and more volatile technique list. Technique IDs are **not** similarly given human-readable names — doing that properly would mean bundling real ATT&CK content (a data file, a licensing note, staleness risk against upstream updates), which was explicitly decided against when this was scoped. Don't "fix" this by adding one without raising it as a scope decision first.
  - `analyze_coverage(technique_id=None)` aggregates the same per-record technique/tactic tag counts as the two functions above, but as a single tool-shaped call: no `technique_id` returns a full breakdown sorted ascending by `rule_count` (weakest-covered first); with one, a focused `{rule_count, covered, mitre_url}` answer for just that technique. An id with no installed rules returns `covered: False` rather than raising — "not covered" is an expected, structured answer for a coverage query, not an error condition (same philosophy as `get_attack_technique`'s empty-list-not-exception behavior for an unmatched id). Every response carries a `coverage_scope` string spelling out that this is coverage over the installed rule set, not a gap analysis against the full ATT&CK matrix.
  - `suggest_rule(query, technique_id=None)` ranks rules by a simple term-overlap score (title match weighted above tags, tags above description) rather than `get_hayabusa_rules`'s exact-substring, unranked, exhaustive match — it answers "what's the closest existing rule," not "list every rule matching X." It surfaces existing rules only; it does not generate new Sigma rules (out of scope — Sigma authoring needs judgment the calling model already has, this tool's job is just finding what already exists).
  - `hayabusa.py`'s `get_hayabusa_rules` does **not** call into this module (and vice versa) — the two have separate, small, near-identical YAML-scanning loops rather than a shared abstraction, to avoid a cross-module dependency for what's still a pretty small amount of duplicated code. If you touch the rule-parsing shape in one, check whether the other needs the same fix.

- **`server.py`** — thin `FastMCP` wrapper (from the official `mcp` SDK) that registers each `hayabusa.py` function as an `@mcp.tool()`, and each `knowledge.py` function as either an `@mcp.tool()` (`analyze_coverage`, `suggest_rule` — parameterized, actively invoked, results reasoned over in one call) or an `@mcp.resource()` under a `hayabusa://` URI (the passively-browsable catalog/coverage views — static resources for the aggregate views, `{param}`-templated URIs like `hayabusa://rules/{rule_id}` for per-item lookups, FastMCP auto-detecting a template from a `{...}` in the URI). Tool docstrings and resource `description=` strings are what the MCP client sees — keep them accurate when changing signatures. `main()` runs the server over stdio.
...
### Adding a new `hayabusa://` resource

1. Add a function in `knowledge.py` that returns a dict (FunctionResource JSON-encodes non-str/bytes returns automatically) — reuse `load_rule_catalog()`/`_resolve_rules_dir()` rather than re-scanning the rules directory yourself.
2. Register it in `server.py` with `@mcp.resource("hayabusa://...", name=..., description=..., mime_type="application/json")`. Use a `{param}`-templated URI (matching the function's parameter name exactly) for per-item lookups.
3. Add a test in `tests/test_knowledge.py` using real small YAML fixtures under `tmp_path` (no mocking — this module only touches the filesystem).
4. Add a case to `tests/test_server.py` asserting the resource/template is registered (`server.mcp.list_resources()` / `list_resource_templates()`) and that reading it (`server.mcp.read_resource(uri)`) routes to the `knowledge.py` function.
...
Tests never invoke the real `hayabusa` binary, and are split across four files by what they exercise:
...
- **`tests/test_knowledge.py`** — `knowledge.py`. Same style as `get_hayabusa_rules`'s tests: no mocking, real small Sigma-style YAML fixtures written into `tmp_path`, asserting against actual parsed/aggregated output. Covers `load_rule_catalog` (valid + malformed rules), the technique/tactic tag regexes (technique vs. sub-technique vs. tactic vs. unrelated tag like `cve.2021-1675` vs. excluded group/software tags like `attack.g0007`), `rules_index` grouping/bounding, not-found behavior (`KeyError`) for `get_rule`/`get_attack_technique` on an unknown id, the enrichment helpers (`_mitre_url`, `_tactic_display_name` including its fallback for an unrecognized tactic), `analyze_coverage` (full breakdown, the `technique_id`-not-covered case returning a structured `covered: False` rather than raising, and per-list bounding via `max_items`), and `suggest_rule` (title-match-outranks-description-match ordering via a small dedicated fixture, `technique_id` pre-filtering, `max_suggestions` bounding, and the empty-`query` `ValueError`).
...
- **`tests/test_server.py`** — the `@mcp.tool()` and `@mcp.resource()` registrations in `server.py`, i.e. the actual integration surface MCP clients call. Monkeypatches the `hayabusa.*`/`knowledge.*` functions directly (not `_run`) and drives them through the real `FastMCP` instance via `asyncio.run(...)` calls to `list_tools()`/`call_tool(name, arguments)` and `list_resources()`/`list_resource_templates()`/`read_resource(uri)`, asserting that each tool/resource is registered and that arguments are forwarded to the wrapped function unchanged — this is what catches a signature/kwarg drift between a wrapper and the function it wraps.
