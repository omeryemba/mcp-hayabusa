...
- **`server.py`** — thin `FastMCP` wrapper (from the official `mcp` SDK) that registers each `hayabusa.py` function as an `@mcp.tool()`, and each `knowledge.py` function as an `@mcp.resource()` under a `hayabusa://` URI (static resources for the aggregate views, `{param}`-templated URIs like `hayabusa://rules/{rule_id}` for per-item lookups — FastMCP auto-detects a template from a `{...}` in the URI). Tool docstrings and resource `description=` strings are what the MCP client sees — keep them accurate when changing signatures. `main()` runs the server over stdio.
