...
**The T1003.003/`ntdsutil` "false negative fix" from commit `8e648e7` was wrong and should be reverted or corrected.** Re-running this investigation and directly testing against the real evidence file (`dc_applog_ntdsutil_dfir_325_326_327.evtx`) shows:
...
**Recommended follow-up (detection-engineering task, not done inline here):** either delete `rules/ntds_dit_extraction_via_ntdsutil_esent.yml` (the builtin already covers this technique correctly and the custom rule adds no working detection) or rewrite its `selection_path` block to a single `Data|contains: ntds.dit` matching the builtin. If kept, update its description/testcases to stop claiming the builtin has a false negative, and add a real executed-detection test (via `hayabusa_csv_timeline` against a fixture `.evtx`, not just YAML-shape validation) to `validate-rule.py`'s check set or this project's test suite, so a rule that structurally can never fire doesn't pass review again.
